...
description: Safely diagnose this repository's gimbal serial link, packet framing, CRC, timing, quaternion interpolation, and reconnect behavior. Use for /dev/gimbal failures, stale state, CRC errors, dropped packets, or incorrect control data.
---
...
5. For angle or timing symptoms, compare the serial timestamp, `Gimbal::orientation_at()` interpolation interval, radians conversion, and the configured `R_gimbal2imubody`. Do not infer a calibration fault from a CRC or transport failure.
...
- Treat `skip_gimbal_crc` as a diagnostic exception only; report it and restore the normal CRC expectation after the experiment.
